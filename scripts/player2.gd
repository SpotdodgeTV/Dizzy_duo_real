...
func _on_area_2d_body_entered(body: Node2D) -> void:
	if body.is_in_group("enemy"):
		...
		if player_one.player_two_connected:
			...
			sling((global_position - body.global_position).normalized(), 5, global_position, false)
...
func _on_body_col_body_entered(body: Node2D) -> void:
	if body.is_in_group("object") or body.is_in_group("enemy"):
		...
		if player_one.player_two_connected:
			...
			print("test")
			sling((player_one.global_position - global_position).normalized(), 2, global_position, false)
